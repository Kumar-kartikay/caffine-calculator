...
export interface CaffeineSource {
  name: string;
  servingSize: string;
  servingSizeMetric?: string; // Add metric serving size
...
// Conversion function for oz to ml (Indian standard)
export function ozToMl(oz: number): number {
  return Math.round(oz * 29.5735); // Standard conversion for India
}

export function calculateCaffeine(inputs: CaffeineInputs): CaffeineResult {
  // Convert weight to kg if needed
  const weightInKg =
    inputs.weightUnit === "lb" ? inputs.weight * 0.453592 : inputs.weight;
  ...
  const baseCaffeine =
    baseMgPerHour * inputs.hoursToSurvive * toleranceModifier;
  ...
  // Total
  const totalMg = Math.round(
    baseCaffeine + weightCaffeine + sleepDeprivationBoost
  );

  // Define caffeine sources with both oz and ml measurements
  const caffeineSources: CaffeineSource[] = [
    {
      name: "Coffee",
      servingSize: "8 oz brewed",
      servingSizeMetric: "240 ml brewed",
    ...
    },
    {
      ...
      servingSizeMetric: "30 ml shot",
      caffeinePerServing: 63,
      servingsNeeded: 0,
    },
    {
      name: "Energy Drink",
      servingSize: "16 oz (Monster)",
      servingSizeMetric: "473 ml (Monster)",
  ...
  return {
    totalMg,
    breakdown: `Base: ${Math.round(baseCaffeine)} mg + Weight: ${Math.round(
      weightCaffeine
    )} mg + Sleep Deprivation: ${Math.round(sleepDeprivationBoost)} mg`,
